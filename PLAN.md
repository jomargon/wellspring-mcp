...
3. **Post-connect success page:** after the Withings callback, show "Connected ✓ — go back to Claude and try: *How did I sleep last week?*". Giving users their first prompt eliminates the "now what?" moment, which is the single biggest drop-off point for MCP connectors.
...
5. **Disconnect:** document both sides — remove the connector in Claude, and revoke access in the Withings account dashboard. Implement token revocation (Withings `revoke` endpoint, requires the signature helper) triggered when a user deletes the client-facing OAuth grant, so "disconnect" genuinely severs access rather than orphaning a live token. This is also an approval signal (see §10).
